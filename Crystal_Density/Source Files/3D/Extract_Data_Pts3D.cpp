...
void Extract_Data_Pts3D ( string const& directory, Input3D const& input, Cell3D const& cell, int index )
{
    ...
    if (input.T2) dir = directory + "Data/T2 Packing Functions/" + input.type_of_experiment + "/Dataset/" + to_string( index ) + ".txt";
    else dir = directory + "Data/Results.txt";
    
    if (input.experimental_T2) dir = directory + "Data/T2 Packing Functions/" + input.type_of_experiment + "/Synthesised Crystals/experimental.txt";
